...
class Horizontal extends AbstractDecorator
{
    ...
    protected function decorateHeading($fieldset)
    {
        ...
        $panelCollapse = $fieldset->elementFactory->createFromExpression(
            'div#collapse' . ucfirst($fieldset->getForm()->getId()) . $fieldset->getIndex() . '.panel-collapse collapse'
        )->setAttribute(
            'role',
            'tabpanel'
        )->setAttribute(
        ...
        $panelBody = $fieldset->elementFactory->createFromExpression('div.panel-body')->setDecoratedParent(
            $panelCollapse
        );
